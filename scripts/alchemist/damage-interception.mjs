...
import { alchemistHasPlatformModule } from "./artificer-automation.mjs";
...
export function applyAlchemistDamageInterception(token,{damageItem,force=false}={}){
  const actor=token?.actor??token?.document?.actor;if(!actor||!force&&!alchemistResponsible(actor))return;
...
async function coatingAttack(workflow){
  ...
  if(!actor||!item||item.system?.equipped===false||!alchemistResponsible(actor)||![...(workflow.hitTargets??[])].length||typeof workflow.setBonusDamageRolls!=="function")return;
  ...
  if(coatingUse.get(active.uuid)===turn)return;
  coatingUse.set(active.uuid,turn);
  const type=active.getFlag(MODULE_ID,"alchemistCoatingType");
...
export function registerAlchemistDamageInterception(){
  ...
  Hooks.on("midi-qol.preTargetDamageApplication",applyAlchemistDamageInterception);
