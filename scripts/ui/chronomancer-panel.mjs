...
function interventionButtons(entries) {
  return entries.slice(0, 12).map((entry, index) => {
    ...
    const x = column ? 388 : 132;
    const y = 174 + (row * 31);
    return `<button type="button" class="ne-crono-intervention" style="--slot-x:${x}px;--slot-y:${y}px" data-item-id="${entry.item.id}" title="${entry.item.name} — ${entry.cost} PT">
