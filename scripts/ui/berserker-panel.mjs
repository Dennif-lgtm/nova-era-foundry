...
function panelMarkup(actor) {
  ...
  const techniques = actor.items.filter(item => String(item.getFlag(MODULE_ID, "group") ?? "").startsWith("tecnicas-")).map(item => {
    const cost = Number(item.getFlag(MODULE_ID, "bloodCost") ?? 0);
    const sacrifice = !!item.getFlag(MODULE_ID, "sacrifice");
    return `<button data-action="technique" data-item-id="${item.id}" ${cost > state.points ? "disabled" : ""}><span>${item.name}</span><small>${cost} PS${sacrifice ? " + Sacrifício" : ""}</small></button>`;
  ...
  return `<section class="nova-era berserker-panel ${state.frenzy ? "is-frenzy" : ""}" data-actor-uuid="${actor.uuid}" style="--blood-ratio:${ratio}">
    <div class="ne-berserker-frame">
      ...
      <details class="ne-berserker-techniques"><summary>TÉCNICAS DE SANGUE</summary><div>${techniques}</div></details>
...
class BerserkerPanelApplication extends FoundryApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, { classes: ["nova-era-window", "nova-era-berserker-window"], width: 460, height: 650, resizable: true, minimizable: true, popOut: true }, { inplace: false });
  }
